...
- Multiple small semantic classes on the same element are often preferred to names like `case-study-page__hero` when local component scoping already provides clarity.
...
- Use BEM-style fused names only when they make the code clearer, especially for classes that must travel outside a local SFC context or be targeted globally.
