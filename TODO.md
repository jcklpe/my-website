...
## In Progress
- Performance and optimization — case-study return clipping now matches the visible card while preserving parallax overscan and authored timing; targeted visual QA remains. Earlier static payload/block preparation is implemented as a candidate, with stressed UX benefit still to verify. Prior responsive WebP/readiness work remains in place; no deployment. Scope, evidence, remaining stutter investigation, and interruption handoff: `docs/active-spikes/performance-optimization.md` / `docs/active-spikes/performance-optimization.todo.md`.
