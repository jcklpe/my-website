...
## In Progress
- Performance and optimization — U1/U4/U5 have positive user QA, including no normal-Chrome strobe and smoother port-3002 motion. U6 now prevents viewport-triggered case-study payload and renderer preparation from competing with active featured transitions while preserving immediate image warming, direct-intent preparation, transition choreography, and parallax. Scheduling, project, generated-static, inspection, and browser lifecycle checks pass; the user accepts U6 as overall smooth on port 3002, with no clear incremental difference from U5. Further work should isolate cold-entry or frame-stall bottlenecks before another implementation. Minimal continuation context lives at `docs/active-spikes/performance-optimization.todo.md` → “Fresh-Agent Handoff” and Checkpoints 34–36; conceptual scope remains in `docs/active-spikes/performance-optimization.md`. The user alone deploys.
...
## Next
- Review the remaining performance candidates and targeted rendering/production verification. Preserve the now-selected single-master dot pattern across responsive sizes; do not reopen independent-width bakes as an assumed visual requirement.
