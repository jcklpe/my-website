...
## In Progress
- Performance and optimization — U1/U4/U5 have positive user QA, including no normal-Chrome strobe and smoother port-3002 motion. U6 now prevents viewport-triggered case-study payload and renderer preparation from competing with active featured transitions while preserving immediate image warming, direct-intent preparation, transition choreography, and parallax. Scheduling, project, generated-static, inspection, and browser lifecycle checks pass; incremental visual smoothness beyond U5 awaits human QA on port 3002. Minimal continuation context lives at `docs/active-spikes/performance-optimization.todo.md` → “Fresh-Agent Handoff” and Checkpoint 34; conceptual scope remains in `docs/active-spikes/performance-optimization.md`. The user alone deploys.
