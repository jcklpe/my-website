...
## In Progress
- Animation & motion spike — umbrella for adding characterful motion to a mostly-static site. **Shipped: the reaction-diffusion page skin (the big one — WebGL, cursor + device-tilt interactive, homepage-only), Conway on the Side Projects card, and slit-slip arrows on the "→" CTAs.** Remaining: ambient background motion (Thread A), which is **gated behind a brainstorm with the user about the site's motion budget now that the homepage is no longer static** — the open question is whether that budget is already spent and the value has moved to the still-static interior pages. Also unstarted and speculative: accordion +/− spin, button-hover character. Jank audit dropped. Continues from brand-voice. Active docs: `docs/active-spikes/animation.md` / `docs/active-spikes/animation.todo.md`.
- Deploy performance spike — **main work shipped and confirmed 2026-08-05 ("way faster to deploy").** The Bunny deploy was uploading ~890 files strictly one at a time, re-uploading unchanged media every publish, and aborting the whole run on any transient failure; it now uses a bounded concurrency pool, retry with backoff, and skips media whose checksum already matches the storage zone. Two optional items remain open and unblocking: extending skip-unchanged to the static output, and a `--force` escape hatch. Continues from static-deploy. Active docs: `docs/active-spikes/deploy-performance.md` / `docs/active-spikes/deploy-performance.todo.md`.
