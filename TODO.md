...
## In Progress
- Performance and optimization — case-study return clipping now matches the visible card while preserving parallax overscan and authored timing; user accepted the overall result 2026-09-16. Lodestone's half-width title reflow is mechanically repaired and ready for visual QA, with intentional phone wrapping preserved. Earlier static payload/block preparation is implemented as a candidate, with stressed UX benefit still to verify. Prior responsive WebP/readiness work remains in place. User reports deploying to Bunny; that live artifact has not been independently matched. Scope, evidence, remaining stutter investigation, and interruption handoff: `docs/active-spikes/performance-optimization.md` / `docs/active-spikes/performance-optimization.todo.md`.
