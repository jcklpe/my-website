...
## In Progress
- Performance and optimization — return clipping and Lodestone title wrapping were accepted 2026-09-16. A new normal-Chrome photoplate strobe report is under targeted QA: the local candidate removes perpetual settled card-motion loops while preserving parallax, but the same-browser visual result is not yet confirmed. The unchanged Bunny deployment reportedly feels worse today than yesterday; no agent deployment occurred and no CDN cause is established. A concise new-task handoff, evidence boundaries, and next checks live at `docs/active-spikes/performance-optimization.todo.md` → “Fresh-Agent Handoff”; conceptual scope remains in `docs/active-spikes/performance-optimization.md`.
