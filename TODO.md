...
## In Progress
- Performance and optimization — destination preparation implemented; the reproduced throttled desktop image-readiness gap is repaired in local paired tests using single-master responsive WebPs. Five case-study image families migrated in the backed-up local CMS; animation unchanged, no deployment. Scope, findings, verification limits, and interruption handoff: `docs/active-spikes/performance-optimization.md` / `docs/active-spikes/performance-optimization.todo.md`.
