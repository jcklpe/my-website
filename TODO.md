...
- Production deploy spike — the static/Bunny release is live at `www.aslanfrench.work`, with valid HTTPS and a path/query-preserving apex 301. Public-origin and default social metadata, robots, sitemap, generated `llms.txt`, same-origin media rewriting, cache classes, security headers, verified release retention, artifact-only rollback, multi-route public verification, and hash-locked obsolete-file pruning pass. The two named QA posts now live only in the QA CMS and their retired public routes return 404. Remaining: user-owned live visual QA and an optional, separately approved HSTS decision; initial propagation monitoring and the declined TTL-reduction path are closed. Warmed accessibility/performance work continues in WCAG/SEO pass 2. Active docs: `docs/active-spikes/production-deploy.md` / `docs/active-spikes/production-deploy.todo.md`.
