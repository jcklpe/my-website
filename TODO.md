...
- Production deploy spike — launch the proven static/Bunny delivery path at `aslanfrench.work`. The account-independent artifact boundary is complete: case-study navigation is prerendered, static public-origin metadata is unified, canonicals/`og:url`/robots/sitemap are generated, and a fresh production-shaped artifact passed inspection. Next: confirm apex/`www` policy and account access, create separate production Bunny storage/pull zones without touching DNS, deploy and QA through that hostname, then address headers, release records, rollback, pruning, and deliberate cutover. Continues from static-deploy and deploy-performance. Active docs: `docs/active-spikes/production-deploy.md` / `docs/active-spikes/production-deploy.todo.md`.
