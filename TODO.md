...
- Production deploy spike — the static/Bunny release is live at `www.aslanfrench.work`, with valid HTTPS and a path/query-preserving apex 301. Public-origin metadata, robots, sitemap, generated `llms.txt`, same-origin media rewriting, real upload/purge, and multi-route public verification now pass; DreamHost remains authoritative for DNS and professional email. Next: human launch QA and the operational hardening layer—cache classes, security headers, release records, rollback rehearsal, and safe pruning—plus a decision on two QA-named posts currently published by the public CMS. Continues from static-deploy and deploy-performance. Active docs: `docs/active-spikes/production-deploy.md` / `docs/active-spikes/production-deploy.todo.md`.
