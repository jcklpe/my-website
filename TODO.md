...
- Production deploy spike — launch the proven static/Bunny delivery path at `aslanfrench.work`. The next implementation slice is deliberately account-independent: prerender self-contained case-study navigation, unify public-origin metadata, generate canonicals/robots/sitemap, and inspect a fresh production-shaped artifact before creating production Bunny zones or touching DNS. Later scope covers DNS/HTTPS and apex/`www` redirect, cache/security headers, release verification, rehearsed rollback, safe remote pruning, and deliberate cutover. Continues from static-deploy and deploy-performance. Active docs: `docs/active-spikes/production-deploy.md` / `docs/active-spikes/production-deploy.todo.md`.
