...
- Production deploy spike — launch the proven static/Bunny delivery path at `www.aslanfrench.work`, with the apex permanently redirecting to it. The account-independent artifact boundary is complete: case-study navigation is prerendered, static public-origin metadata is unified, canonicals/`og:url`/robots/sitemap are generated, and a fresh production-shaped artifact passed inspection. DreamHost remains authoritative for DNS and professional email. Next: inventory account access, create separate production Bunny storage/pull zones without touching DNS, deploy and QA through the Bunny hostname, then configure custom hostnames/SSL and the apex redirect before addressing headers, release records, rollback, pruning, and deliberate cutover. Continues from static-deploy and deploy-performance. Active docs: `docs/active-spikes/production-deploy.md` / `docs/active-spikes/production-deploy.todo.md`.
