...
`STATIC_PUBLIC_SITE_URL` is the single public-origin input for static generation. It supplies self-referential canonicals, `og:url`, `robots.txt`, and `sitemap.xml`; shell values override the ignored `.env.deploy` file. A public-CMS build is indexable only when `STATIC_DEPLOY_ENV=production`, while preview and QA builds emit `noindex` metadata and a crawler-blocking `robots.txt`. The intended production values are `STATIC_DEPLOY_ENV=production` and `STATIC_PUBLIC_SITE_URL=https://www.aslanfrench.work`; the apex hostname redirects permanently to the matching `www` path.
