...
- **Bypass and route announcements**: The default layout's first-focus skip link targets the focusable `main#main-content`, and `app.vue` mounts `NuxtRouteAnnouncer` for client-side route changes. Preserve both when changing the application shell.
...
- **Image alt**: `FeaturedMediaFrame` emits CMS alt text or an empty alt string (never omits the attribute). Block images preserve WordPress alt attributes. Mega Gallery image buttons use image alt in their accessible label. In WordPress, give informative images concise contextual alternatives in both Media Library metadata and existing Gutenberg image instances; use empty alt only when an image is decorative in that context.
- **Editorial media accessibility**: Give embedded frames content-specific purpose names rather than repeated provider-generic titles. Captions, transcripts, and complex-image descriptions require source knowledge; record missing assets for editorial follow-up instead of fabricating them in frontend code.
...
- Keep clone geometry, source/destination page visibility, handoff ownership, and card-frame styling separate. The handoff is deliberately overlapped: A is the seated teleported clone and B is the real destination; B must enter to full opacity while A remains mounted at full opacity, the browser must paint one frame containing both, and only then may A leave. Keep `active`, geometry `phase`, and `handoffPhase` separate. Never simplify this into one update that reveals B while fading/removing A—the redundancy is intentional art direction and prevents transparent frames, image bleed, and mismatched title-ground colors.
- Preserve the interior transition stacking order: teleported text clone above the real destination header/content, and the real destination header/content above the teleported media clone. Homepage reaction-diffusion isolation must not trap interior page grounds beneath the media clone.
