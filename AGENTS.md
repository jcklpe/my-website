...
- The Home page uses ACF fields for structured homepage content: the vital-info tagline, quick links, employer testimonials, testimonials background texture, and the hero portrait image (`hero_portrait`, exposed as `homepageHeroPortrait` / `homepageHeroPortraitAlt` on the `Page` GraphQL type). Its Gutenberg body editor is intentionally hidden. The BLUF hero portrait is CMS-swappable and falls back to a built-in mock (`/images/home-portrait-mock.webp`) when unset; the responsive composition is tuned to a ~4:5 portrait crop.
