...
The command should be explicit about what it starts. A good first shape would probably run the Docker Compose stack that includes both public and QA CMS containers, then run the Nuxt frontend on `127.0.0.1:3001`. It should not hide errors from either process, and it should avoid making DDEV or another runtime canonical by accident.
