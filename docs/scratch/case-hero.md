...
Open spike. Not blocking the active design branch (`gendes-blue2.claudecode`). The "slip panel" approach is the working interim solution. This doc captures the problem and the three candidate directions so we can dig in deliberately later.
...
- Cleanest typographic surface — works with any of the Blueprint Brutalism vocabulary (panel header, mono labels, periwinkle rule).
...
Doesn't fit the current Blueprint Brutalism direction (the vocabulary is structural panels, not tape labels). Worth noting because it's the only option that works *over any image* without art-direction cost. If the site ever moves in a more underground/zine direction, this comes back.
...
**Option 1 alone is the most ambitious.** It would mean inventing a filter that becomes a visual signature — something like a duotone treatment that *says* "this is a Blueprint Brutalism case study card." Worth designing if we have the energy and the right filter idea presents itself.
