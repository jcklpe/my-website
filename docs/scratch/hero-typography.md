...
Open spike draft. Not blocking the active design branch (`gendes-blue2.claudecode`). The current hero composition in `apps/frontend/pages/index.vue` is a hand-sketched first cut using absolute pixel positioning. It looks right at the width it was tuned at; this spike is about turning that sketch into a robust, responsive, intentional piece of display typography.

## Background

The homepage hero is the front door of the site. The direction (Blueprint Brutalism — synthesis) calls for an expressive, art-directed typographic moment that pairs three voices:
...
- The family CSS variables (`--font-script`, `--font-display-serif`) are defined explicitly with quoted strings in `packages/styles/context-role/_vue-frontend.scss` — they can't be SCSS-interpolated because "Bodoni Z37" contains a numeric token that breaks unquoted CSS font-family parsing.
...
- `packages/styles/_type-palette.scss` — `$font-script`, `$font-display-serif` Sass variables
- `packages/styles/context-role/_vue-frontend.scss` — `--font-script` / `--font-display-serif` CSS variables (explicitly quoted)
...
- `docs/gendes-brief.md` — the synthesis brief, Typography section, describes the BLUF experiment
