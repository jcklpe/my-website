...
2. **Mood board** — on that branch, assemble the mood board under `docs/gendes-moodboard/<branch-name>/`. If a written direction note is useful, put it as a `notes.md` inside that branch-specific folder rather than a separate brief file. The direction should be specific enough to guide real decisions: what surfaces look like, what the motion personality is, what typographic voice is being explored.
