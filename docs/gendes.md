...
The brief location is an intentional active input. Create the design branch first, then fill the brief on that branch before a generative design agent begins implementation. A branch can use a traditional mood board, a single website/product reference, or another concrete source of visual truth. Mood-board media is ignored by Git; use branch-named folders so local reference files can coexist while switching branches when a branch needs local media.
...
2. **Brief and references** — on that branch, pick a mood/direction and write the brief in `docs/gendes-brief.md`. Generate or assemble a mood board under `docs/gendes-moodboard/<branch-name>/` when useful, or name the website/object/interface references that should guide the branch. The brief should be specific enough to guide real decisions: what surfaces look like, what the motion personality is, what typographic voice is being explored.
