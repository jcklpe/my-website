...
The brief and mood-board locations are intentional active inputs. Create the design branch first, then fill the brief and direction-specific mood-board folder on that branch before a generative design agent begins implementation. Mood-board media is ignored by Git; use direction-named folders so local reference files can coexist while switching branches.
...
2. **Brief and mood board** — on that branch, pick a mood/direction and write the brief in `docs/gendes-brief.md`. Generate or assemble the mood board under `docs/gendes-moodboard/<direction>/`. The brief should be specific enough to guide real decisions: what surfaces look like, what the motion personality is, what typographic voice is being explored.
...
4. **Review** — the design branch should be reviewed as a real rendered site, not a static mockup. SSR via the normal dev stack remains useful, but the current preferred visual-QA path for gendes branches is static generation plus local static preview: generate from the chosen CMS source, then run `corepack pnpm start:static:preview`.
