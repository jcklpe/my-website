...
2. Each design branch gets a mood board, and an agent-driven implementation pass.
...
The mood-board location is an intentional active input. Create the design branch first, then fill the branch-specific mood-board folder on that branch before a generative design agent begins implementation. Mood-board media is ignored by Git; use branch-named folders so local reference files can coexist while switching branches.
...
2. **Mood board** — on that branch, assemble the mood board under `docs/gendes-moodboard/<branch-name>/`. If a written direction note is useful, put it as a `notes.md` inside that branch-specific folder. The direction should be specific enough to guide real decisions: what surfaces look like, what the motion personality is, what typographic voice is being explored.
...
The goal is not just to twist theme knobs. Palette and type changes are expected, but a strong design branch should also ask compositional questions: how sections are staged, how cards behave as objects, how reading surfaces feel, how media enters the page, and how the site creates a memorable rhythm across homepage, archive, and article/detail views. Agents should be encouraged to move beyond token substitution when the visual direction calls for it, while preserving the content and rendering contracts.
