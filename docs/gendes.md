...
The brief is the required creative input. A branch may use a local mood-board folder, a live website reference, or both. Create the design branch first, then fill the brief and any branch-specific reference folder on that branch before a generative design agent begins implementation. Mood-board media is ignored by Git; use branch-named folders so local reference files can coexist while switching branches.
...
2. **Brief and references** — on that branch, pick a mood/direction and write the brief in `docs/gendes-brief.md`. Generate or assemble a mood board under `docs/gendes-moodboard/<branch-name>/` when useful, or name a live website/reference object directly in the brief. The brief should be specific enough to guide real decisions: what surfaces look like, what the motion personality is, what typographic voice is being explored.
...
4. **Review** — the design branch can be reviewed through the normal SSR dev stack or through the explicit static preview path (`corepack pnpm generate:static:*` + `corepack pnpm start:static:preview`). Use the static preview path when the experiment is meant to test the publishable artifact. Switch between branches in the same repo to compare directions.
