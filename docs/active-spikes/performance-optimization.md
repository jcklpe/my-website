...
Latest outcome: the clone now interpolates the visible card clipping bounds while retaining the oversized image geometry; local browser geometry verifies the reverse landing mismatch is repaired. The companion todo contains a targeted visual QA item and portable verification evidence. An earlier static payload/block-renderer preparation candidate is implemented, but a stressed UX benefit remains unproven. Public McMaster-Carr category/filter/back navigation succeeded without login. Read the companion Fresh-Agent Handoff before the historical sections below.
