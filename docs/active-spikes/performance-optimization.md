...
Latest outcome: the clone now interpolates the visible card clipping bounds while retaining the oversized image geometry; local browser geometry verifies the reverse landing mismatch is repaired. The user accepted the overall result on 2026-09-16. The subsequently reported Lodestone title reflow is also repaired: case-study title clones preserve the endpoint's desktop or phone wrapping rule throughout the flight, with bounded browser verification complete and visual QA pending. The companion todo contains portable verification evidence. An earlier static payload/block-renderer preparation candidate is implemented, but a stressed UX benefit remains unproven. Public McMaster-Carr category/filter/back navigation succeeded without login. The user also reports generating/deploying to Bunny and a slightly less smooth live experience than the internal preview; this is not an independently verified artifact comparison. Read the companion Fresh-Agent Handoff before the historical sections below.
