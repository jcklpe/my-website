...
- **Text as moving material:** candidates included low-amplitude displacement ooze, animated masks, print-registration copies, variable-font axes, per-letter choreography, and purpose-built SVG glyphs. Human review narrowed the Selected Work accent itself to the irregular travelling flag, repaired mass-shedding metaballs, and a hybrid whose moving curves throw slime-like vector droplets across the rope rather than only from its tip. Preserve semantic text and confine procedural material to small display/accent regions.
...
- **Testimonial motion and quotation grammar:** testimonials remain continuously present. Their header joins the selected pointer-proximity field; the CMS texture responds to pointer or supported device-orientation input; the accepted striped block signal keeps travelling; and a large Bodoni opening mark carries a stronger slow color wash. Additional card padding and a quotation gutter reduce direct mark/text collision without flattening the layered composition. Final readability remains a human QA call.
- **Section-level ambient rhythm:** each major homepage region should have one quiet repeating or input-responsive behavior with a compositional role, not the same widget everywhere. Selected Work has image depth/catalog reveal, a broad occasional blue current, and the unresolved ordinal finalist; Testimonials has quotation, texture, and block signals; Side Projects has Conway plus heading depth; Latest Writing has low-travel card proximity and crosshair rotation; the footer has its quiet edge signal and clipped CMS-heading ticker. Latest Writing may still need one non-pointer ambient behavior inside the bento itself.
...
- **Interior organisms:** the independent CPU margin-patch approach is rejected. It advanced the chemistry far more slowly than the homepage GPU field while consuming enough main-thread time to compromise article and transition performance. If the idea returns, implement one shared GPU field revealed through measured page-level masks or scissor windows; do not multiply standalone simulations.
