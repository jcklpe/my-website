...
## Status
Active, approaching closeout. The production motion choices are settled. Remaining comparisons concern article margin placement and optional ghost-image transition trails. The reopened title-ground blink is human-confirmed fixed by the explicit A/B overlap model, and current phone browser-Back testing passes. Preserve the historical explorations below and in the to-do doc.
...
- **A — Ambient background motion:** the brainstorm produced a CMS-backed `/dev/motion` comparison lab and moved selected candidates into iterative homepage/interior-page QA. Catalog shuffle with practice/discipline plus engagement context, inset-image parallax, colour reveal, and baked halftone are selected for the case-study cards. Pointer proximity is selected as the homepage display-heading model and now applies to Selected Work, Latest Writing, Testimonials, and Side Projects without a debug toggle. Broad viewport entry/exit choreography, the testimonial lateral wipe, CPU margin organisms, and the Latest Writing ambient-card candidates are rejected and removed. The writing archive's 140ms bottom-slit sequence remains selected. The irregular SVG flag is the selected Selected Work accent; its possible repetition as a broader accent grammar remains an optional future design question rather than a closeout blocker.
- **E (accordion spin):** already existed before this spike; it is not outstanding work. **F (button hover):** the speculative accent-flash/fill candidate was never selected and is dropped.
- **G — jank audit:** originally dropped 2026-07-29, then reopened for later regressions. The title-ground blink is resolved through explicit overlapping ownership, documented near the implementation. Final checks concern the other navigation paths; no replacement of the working handoff is planned.

The homepage combines the approved RD/Conway fields with pointer-proximity headings, the irregular flag, rotating case-study stars, catalogue/parallax hover, testimonial signals, and footer motion. Writing details now trial the actual GPU ecology in desktop margins after 150vh of scrolling. The reading column and authored blocks retain cream clearing; empty margins must expose the field. Treat RD as a subtle baseline material and judge additions by their actual coexistence, legibility, performance, and distinctness rather than a predetermined count of animated surfaces.
...
- **Featured-media route transition** (`useFeaturedMediaTransition.ts`, `useHomeTransitionChoreography.ts`): the custom card↔detail image+title morph across navigation, coordinated through Vue state and CSS transitions. The optional dev-only image trails use native Web Animations. This is "the most considered part of the whole system" and the transition system **wins**. Its timing lives in the Sass **motion palette** (`packages/styles/_motion-palette.scss`), exported as CSS custom properties, and read by JS where cleanup timing must match CSS. See AGENTS.md → "Route Transition and Motion Rules" and `docs/design-system.md`.
- **HARD RULE — preserve the overlapped featured-media handoff.** The permanent destination fades fully in beneath the still-opaque seated clone, both are held through a committed paint, and only then does the clone leave. `active`, geometry `phase`, and `handoffPhase` intentionally remain distinct; do not refactor this into a single visibility toggle even though the duplication looks less tidy.
...
- **Text as moving material:** candidates included low-amplitude displacement ooze, animated masks, print-registration copies, variable-font axes, per-letter choreography, and purpose-built SVG glyphs. Human review selected the irregular travelling SVG flag for the Selected Work accent. Standalone WebGL shedding lava was impressive but never quite resolved its clipping, looping, and material-shape problems; several hybrids explored particles, explicit tendrils, implicit-field shedding, rightward autonomy, and recapture, but none made shedding and reformation feel physically coherent enough. Those branches are removed from production and Motion QA. Preserve semantic text and confine procedural material to small display/accent regions.
...
- **Testimonial motion and quotation grammar:** testimonials remain continuously present. Their header joins the selected pointer-proximity field; the CMS texture responds to pointer or supported device-orientation input; the accepted striped block signal keeps travelling; and a large Bodoni closing mark sits in the lower-right corner with a stronger slow color wash. Additional card padding lets the mark remain layered without crossing the quote body. Final placement and readability remain a human QA call.
- **Section-level ambient rhythm:** each major homepage region can carry one quiet repeating or input-responsive behavior with a compositional role, but motion is not a quota. Selected Work has image depth/catalog reveal, a broad occasional blue current, and the approved rotating ordinal star; Testimonials has quotation, texture, and block signals; Side Projects has Conway plus heading depth; Latest Writing has low-travel card proximity and crosshair rotation; the footer has its quiet edge signal and clipped CMS-heading ticker. The edge relay and five subsequent autonomous bento-card candidates—corner constellation, date beacon, image blue tide, registration drift, and border breathing—were all rejected. The absence of another per-card effect is an intentional stopping point rather than a gap.
- **Accent-rule coherence:** the Selected Work rule is not approved merely because its remaining texture modes are interesting. A one-off fluid mark can read as a rendering mistake when no other surface shares its grammar. The accent is now stable while heading letters respond to the pointer, because moving both made them read as one gimmick rather than a signal plus a field. Explore a family of small signal/material marks at genuine structural punctuation; repeat the grammar, not necessarily the same renderer.
- **Entrance and transition precedence:** the writing archive treats blue year headers and article rows as one ordered sequence. On a reverse article-to-archive transition, the active article row is the anchor beat: nearby earlier elements finish before the shared-media flight lands, its row finishes with the landing, and later elements follow. Clip-path preserves measured geometry. Direct and homepage arrivals use the same ordered elements without waiting on shared-media state. Writing-card transitions must not inherit the case-study-only surroundings delay.
- **Interior organisms:** the independent CPU margin-patch approach is rejected. It advanced the chemistry far more slowly than the homepage GPU field while consuming enough main-thread time to compromise article and transition performance. The bounded `/dev/margin-organisms` mock demonstrated the viable successor architecture by revealing one actual homepage GPU field through two soft margin windows, and human review approved that visual for a production trial. Writing-detail pages now mount the same single field behind their opaque content surfaces, reveal it only through desktop margin masks, and pause it during featured-media transitions. Keep this shared-field model if the trial survives; do not multiply standalone simulations.
...
**Micro-interaction taste:** the case-study hover grammar and known slit-slip arrow targets are settled. The speculative button accent-flash/fill idea was never selected and is dropped. Accordion rotation already exists and is not an open implementation candidate.
