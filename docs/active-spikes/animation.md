...
- **A — Ambient background motion:** the brainstorm produced a fake-content `/dev/motion` comparison lab, now ready for Human QA. It keeps production cards untouched while comparing the full case-study-hover slate through one global selector, alongside display-text motion, a tiny RD eyebrow, inset depth, staggered entrances, and sparse interior organisms. The lab is a taste-selection surface, not approval to ship its candidates.
...
- **Case-study hover as a prototype grammar:** the current treatment mostly removes the colour filter. Human QA selected the combination of clipped catalog metadata, inset parallax, and colour reveal for eventual production integration; retained print misregistration only as a comparison; and rejected moving partitions, aperture reveals, and project-specific signals. Image inset parallax is an independent layer rather than a mutually exclusive treatment so it can be judged alone, with catalog metadata, or with print registration. The catalog has two defined editorial roles: a short **practice/discipline** line describing the work performed (for example, “Design systems · Operations”) and a short **engagement context** line describing the situation or intervention (for example, “Cross-department product delivery”). If integrated, these should be explicit CMS fields rather than inferred categories or decorative status copy. Keep the measured outer media/title/slip shells stable; inset parallax and other motion should occur on inner layers that can settle during the existing preflight beat before transition geometry is captured. Keep the existing duotone-to-colour hover independently switchable while tuning so its interaction with each candidate stays visible. The original halftone rationale—reducing gamut enough to support overlaid text—did not hold up in practice, and the separate text plate solved legibility while also enabling compositional variance. Reassessing halftone is therefore valid, but any future plate alternative must preserve that variance rather than collapsing Selected Work into identical image boxes; compare untreated imagery with localized scrims, authored negative-space placement, or other asymmetrical image/text relationships before replacing the current plate model.
