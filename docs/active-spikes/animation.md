...
- **Case-study hover as a prototype grammar:** the current treatment mostly removes the colour filter. Human QA selected the clipped catalog/metadata shuffle for eventual production integration, retained print misregistration for comparison, and rejected moving partitions, aperture reveals, and project-specific signals. Image inset parallax is now an independent layer rather than a mutually exclusive treatment so it can be judged alone, with catalog metadata, or with print registration. The selected catalog direction must carry useful project-discipline information rather than decorative labels, which will require a small settled CMS vocabulary before adding an ACF field. Keep the measured outer media/title/slip shells stable; inset parallax and other motion should occur on inner layers that can settle during the existing preflight beat before transition geometry is captured. Keep the existing duotone-to-colour hover independently switchable while tuning so its interaction with each candidate stays visible.
