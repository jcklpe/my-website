...
- **Case-study hover as a prototype grammar:** the current treatment mostly removes the colour filter. Build the candidates first on a fake-content motion lab where every card uses the same currently selected treatment and a floating debug control changes the treatment globally. The full comparison set includes moving internal partitions, print-misregistration layers, aperture reveals, clipped catalog/metadata shuffles, and project-specific animated signals. Keep the measured outer media/title/slip shells stable; inset parallax and other motion should occur on inner layers that can settle during the existing preflight beat before transition geometry is captured. Do not add a CMS motion field until a winning treatment and actual per-project need emerge from the lab.
