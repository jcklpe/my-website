...
- Homepage reaction-diffusion field: evaluate as an alternative ambient layer rather than assuming it belongs with the portrait. Determine whether a deterministic low-resolution ink/blue field can fade beyond the hero rectangle, remain quiet behind typography, pause offscreen, provide a static reduced-motion state, and stay cheap on mobile. Do not use motion as a substitute for solving the hero's composition.
- Homepage hero motion: decide whether this belongs in this spike or should continue into `docs/scratch/animations.md`.
- Featured-media transition jank audit: reproduce case-study and writing transitions separately in forward and reverse directions, with cold and warm caches and at desktop/phone widths. Record whether the visible failure is source disappearance, flying-clone geometry, title wrapping/slip timing, destination reveal, scroll landing, or frame handoff before changing duration/easing. Route the resulting implementation into the animation spike if it is broader than brand-voice QA.
