...
First implemented portrait candidate 2026-07-14: use the existing blue-and-cream About portrait rather than generating a new asset. On desktop, narrow the wordmark field toward the left, place the portrait as a tall raised sheet on the right, and let the Vital Info sheet overlap its lower-left edge so all three elements form one opening composition. On phone, retain the portrait/Vital overlap as a provisional in-flow arrangement; do not treat this as resolving the separate mobile BLUF recomposition, since the inherited desktop wordmark still clips and needs deliberate vertical art direction. Human QA should judge whether the portrait's stronger self-centering feels appropriate, whether the sheet is too visually dominant, and whether the overlap reads as one composition rather than three stacked objects.
