...
Tablet + phone recomposition 2026-07-28: the tablet lockup was iterated to Human-QA approval and the phone mode was re-directed entirely. Phone is no longer three vertical lines but a “framed portrait” magazine-cover composition, chosen over a stacked-lockup alternative during Human QA: a large centered portrait is the anchor, “Line” runs horizontally across the top, “Bottom” is a sideways script column down the portrait's left edge, and `UP FRONT` is an upright serif spine down its right edge, with the BLUF badge tucked into the top-right negative space and the blueprint gradient restored full-width behind the top. The direction deliberately fragments and rotates the “Bottom Line Up Front” phrase — a knowing irony against the concept's “get to the point” meaning — in exchange for a bolder, art-directed opening that uses the tall phone canvas and makes the author's portrait the subject. Two durable techniques came out of making it hold across widths. First, the whole phone composition is anchored to a single ruler: a `--phone-stage` custom property (`min(90vw, 38rem)`, i.e. the portrait width), with every size, position, and the headroom expressed as a `calc()` fraction of it, so the pieces scale as one rigid unit and cap together instead of diverging the way viewport-relative (`vw`) type does once it outgrows a `rem`-capped portrait. A first attempt using a CSS container (`container-type: inline-size`) with `cqw` units collapsed the portrait to a thumbnail, so the calc-fraction form is preferred here. Second, on tablet the “Selected work” label was decoupled from the case-study cards by giving it `height: 0` (its title overflows visibly) so the cards stop reacting to the scaling heading, and the Vital Info card was pinned to a constant height by fixing the tagline's wrap width — together these ended a long run of per-width position drift where the label kept sliding behind the vital card or the cards. Vertical rotated text uses `writing-mode` + `text-orientation` deliberately: `sideways` for the connected script (“Bottom”) so its joins survive, `upright` for the serif spine (`UP FRONT`); in vertical text it is negative `letter-spacing`/`word-spacing` (not `line-height`) that crushes the inter-letter gaps.

Hero settled across all breakpoints 2026-07-29: the wide-desktop lockup was polished and the whole responsive BLUF hero was Human-QA approved from narrowest phone through widest desktop. The one durable lesson worth carrying beyond this spike — it recurred at every breakpoint — is that mixing a viewport-scaled unit with a fixed one is what breaks compositions at width: `vw`/`cqw` type against a `rem`/`px` position (or a fixed `-50px` raise against a viewport-scaled wordmark) tracks only at the single width it was tuned at and drifts everywhere else. The fixes are always the same shape: put both on one ruler (the phone's single `--phone-stage` with `calc()` fractions), scale the offset too (Vital's width-scaled `clamp(-42px, calc(40px - 4.2vw), 8px)` raise), or exploit a coincidence where two things already share a slope (the desktop portrait border and the `UP FRONT` T both ≈`0.66×viewport`, so a constant offset holds their alignment). The one open follow-up from the whole hero effort is asset provenance: the portrait is still a provisional copied derivative and needs a canonical CMS/shared-media source before this is truly finished.
