...
- Production origin configuration, self-canonicals, `og:url`, default social metadata, robots, sitemap, `llms.txt`, target guards, real upload/purge, post-rewrite media verification, multi-route public verification, custom hostnames, SSL, apex redirect, cache classes, security headers, verified release records, and artifact-only rollback are implemented and verified. Actual obsolete-file deletion and human launch QA remain open; warmed accessibility/performance work is routed to the later WCAG/SEO spike.
...
## Ready for Human QA
- H. Complete desktop and phone visual/interaction QA on the live `www` release at the user's own pace, especially route transitions, media/lightbox, TOC/footnotes, and construction/motion controls. This does not need agent attention unless the user reports a defect.
