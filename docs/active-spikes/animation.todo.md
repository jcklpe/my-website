...
- Reaction-diffusion page-skin (Thread B) — FIRST PASS implemented 2026-07-30, expect tuning rounds. New `apps/frontend/components/home/HomeReactionDiffusionBackground.vue`: a fixed full-viewport `<canvas>` (`z-index: -1`, `pointer-events: none`) added as the first child of `.home-page` in `index.vue`, which got `isolation: isolate` so the canvas sits above the paper-grid background and below content. Gray-Scott (coral params `FEED 0.0545` / `KILL 0.062`), simulated low-res (`SIM_SCALE 5`, `MAX_COLS 360` cap) and upscaled smooth for the buttery ooze; pale periwinkle `rgb(150,162,236)` at faint alpha (`MAX_ALPHA 82`). A static value-noise **fertility mask** (`FERTILE_FRACTION 0.34`) makes patterns sustain only in ~a third of the area and decay (`BARREN_DECAY`) elsewhere → wide negative space; a small `GLOBAL_DECAY` keeps growth semi-temporary. Hover blooms under the cursor (window `mousemove`, throttled seeding in the step loop). Paused when the tab is hidden and during featured-media transitions; reduced-motion develops ~90 iterations then draws a single static frame. All the taste knobs are consts at the top of the component. `corepack pnpm check` passes with only the two existing `vue/no-v-html` warnings. Human QA (expect iteration): (1) colour/alpha over the grid paper — pale enough / periwinkle right; (2) sparseness + negative space — is ~a third coverage the right asymmetry, does it leave enough open space; (3) speed — slow buttery ooze vs too fast/slow (`ITERS_PER_TICK`, `TICK_FPS`, `DT`); (4) scale of features (`SIM_SCALE`, diffusion); (5) hover growth reads clearly and fades semi-temporarily without permanent trails; (6) **desktop + mobile perf** (full-viewport sim — the gating concern; may need a coarser mobile `SIM_SCALE` or disabling on phones); (7) placement/stacking is correct (over the grid, under all content, not clipped) and it doesn't interfere with the featured-media transition; (8) static-generated output. Currently homepage-only; promote to a site-wide layout if approved.
