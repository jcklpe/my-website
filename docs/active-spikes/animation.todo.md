...
- A13. READY FOR HUMAN QA: the Selected Work production rule compares the crisp filled-SVG fluid ribbon, irregular and regular travelling waves, supersampled procedural flow, slow lava metaballs, and shedding lava; the rejected domain-warp mode is removed. The QA panel now exposes the real rule viewport independently through wide X/Y, pixel width, and pixel height ranges, while vector thickness, wave amplitude/frequency, and shedding body height/length/dispersion/reach retain independent controls with substantially higher ceilings. WebGL and vector surfaces composite transparently over the page. The mark moves with the pointer field. All modes pause offscreen, during featured-media transitions, and under reduced motion. The remaining design question is whether this mark has a coherent site-wide grammar; interesting shader behavior alone is not approval.
