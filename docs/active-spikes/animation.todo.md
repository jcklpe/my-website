...
B, C, and D shipped. Android reaction-diffusion tilt is approved, the accountless phone-preview workflow remains in Human QA, and the first Thread A implementation slice is the CMS-backed `/dev/motion` comparison lab, now ready for Human QA. No lab candidate is approved for production use yet.
...
- A5. SELECTED FOR PRODUCTION 2026-08-12, integration pending: add the catalog-shuffle hover to Selected Work using useful project-discipline metadata rather than decorative labels. The lab direction is approved. Before production integration, settle the editorial vocabulary and add a small ACF field (or fields) for the metadata, preserve the existing card-to-detail measurement geometry, retain a switchable comparison with the current duotone-to-color hover while tuning, and settle the touch behavior; the current lab activates the centered card as a plausible mobile model.
