...
Re-prioritised 2026-08-03, after Conway (C) and reaction-diffusion (B) both landed: **Thread D (slit-slip consistency) is the next implementation slice** — it is well-specified and low-risk. After D, Thread A (further ambient motion) needs a **brainstorm/discussion phase with the user before any building**. Remaining open items on B are listed under its thread.
...
**IMPLEMENTED + QA-APPROVED 2026-08-03 ("this looks pretty much perfect"). See Done for the full record and the durable lessons. Remaining open items:**
- B5. Mobile motion story. IN PROGRESS. There is no pointer on touch, so the cursor attraction is desktop-only. iOS is settled: `DeviceOrientation` needs a modal permission prompt, which is far too much friction for a background texture, so it is never requested — iOS runs on touch plus a slow wander, Android additionally gets tilt.
  - The first attempt made tilt roll an **influence point** through the field (the Jackalope rellax precedent). It works and is clearly visible in the harness, but on the homepage the coral is deliberately fragmented and perturbed, so a moving point is hard to pick out — QA 2026-08-05.
  - Now reframed to **tilt sloshing the whole fertility field**, like water in a shallow pan: tilt accelerates the drift and drag lets it coast back, so the direction of tilt shifts where the reaction travels instead of moving a marker through it. Moving the entire field is legible even where individual coral is not. Awaiting QA on device.
  - OPEN: the sign convention. Whether positive `gamma`/`beta` should send the field that way or the other is a judgement call that needs eyes on a real device; the harness slider spans zero so it can be flipped there.
- B6. OPEN: mobile perf pass — the sim is full-viewport; confirm cost on a real phone and decide whether to lower `SIM_SCALE` / `ITERS_PER_FRAME` there.
...
- [x] **Thread B — Reaction-diffusion page-wide skin.** Approved 2026-08-03. `HomeReactionDiffusionBackground.vue` — a fixed full-viewport WebGL2 canvas (`z-index: -1`, `pointer-events: none`) over the paper grid, below content, with the footer given its own positive `z-index` so it paints over it.
  ...
  **Durable lessons — these cost a week of wrong diagnoses. Do not relearn them.**
  ...
  7. **Process:** the breakthrough came from building `/dev/rd` — a harness that made the reaction, mask, drift and seeding independently toggleable with raw-field views. Debugging four coupled subsystems through one composited output produced confident, repeatedly wrong theories. **Build the isolation harness early.** It is kept (not deleted) as a possible easter egg / blog toy.
