...
- A13. READY FOR HUMAN QA: the Selected Work production rule still compares seven rendering/textural approaches. `HomeFluidAccentRule.vue` retains the crisp filled-SVG fluid ribbon and two travelling-wave constructions; vector modes expose a 0.5–4× thickness control in addition to amplitude/frequency where relevant. The shared panel now controls the mark's X/Y placement and total rendered length independently from its material. `HomeWebglAccentRule.vue` supplies supersampled procedural flow, slow lava metaballs, domain warp, and shedding lava. The shedding mode exposes independent body height, body length, dispersion, and particle-reach controls; its lobes stretch through temporary bridges, open moving negative space, and release shrinking particles beyond the main mass. The mark moves with the pointer field. All modes pause offscreen, during featured-media transitions, and under reduced motion. The remaining design question is whether this mark has a coherent site-wide grammar; interesting shader behavior alone is not approval.
- A14. READY FOR HUMAN QA: testimonials no longer animate in or out. The title uses the selected pointer-proximity field. The CMS-selected background texture can move as a shallow composited pointer-parallax plane with an enable toggle and strength control. Each quote carries a larger low-contrast Bodoni opening mark behind the text, and the QA panel switches the repeating blue signal between the original travelling card bars and a striped animated quotation mark. Reduced motion freezes the texture, stripes, quote signal, and heading field. Verify quotation clarity, contrast, card hierarchy, texture depth, and which signal placement makes the quotation grammar feel intentional.
