...
- B5. OPEN: mobile motion story. There is no pointer on touch, so the cursor-attraction effect is desktop-only. Candidate: an **accelerometer/orientation-driven "rolling ball"** influence point (precedent: the Jackalope WordPress theme's rellax-style parallax, which fell back from mouse to accelerometer on mobile — `temp-ref-assets/`). Must be very light; needs the iOS 13+ permission-gesture story settled. Discuss before building.
