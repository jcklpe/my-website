...
**Current priority, updated 2026-08-20 — START HERE IF YOU ARE PICKING THIS UP COLD.**

B, C, and D shipped. Android reaction-diffusion tilt is approved, the accountless phone-preview workflow remains in Human QA, and Thread A has its first production decisions from a second-person review. Pointer proximity is the selected heading model and now reaches Side Projects as well as Selected Work, Latest Writing, and Testimonials. Catalog metadata, inset-image parallax, colour reveal, and baked halftone are selected for case-study cards. Broad homepage viewport entry/exit motion, the testimonial reveal, alternate heading geometries, and the original-image comparison are rejected and removed. The writing-row entrance remains in QA. The accent-rule material, RD structural accents, article margin organisms, and broader accent grammar remain open. The current QA controls concentrate on the unresolved accent material and small ambient candidates rather than reopening rejected branches.
...
- A7. READY FOR HUMAN QA: the earlier heading-only negative-Z entrance and scroll-scrubbed departure felt isolated from their otherwise stable sections, so they were replaced by a real-homepage comparison of viewport choreography and seven pointer geometries. Human review on 2026-08-20 selected **pointer proximity** and rejected broad viewport entry/pre-exit motion plus the other six heading models. The comparison controls and rejected implementations are removed. The production field now applies page-relative motion to Selected Work, Latest Writing, and Testimonials; samples stable untransformed letter wrappers to avoid feedback jitter; pauses during featured-media transitions; and keeps overflow visible to prevent the reported glyph-box clipping near the pointer. The Selected Work accent participates in the same field. Verify all words/lines respond, direct pointer overlap does not clip or chatter, tablet compositions remain unchanged, and reduced motion is static.
- A13. READY FOR HUMAN QA: the Selected Work production rule compares the crisp filled-SVG fluid ribbon, irregular and regular travelling waves, supersampled procedural flow, slow lava metaballs, and shedding lava; the rejected domain-warp mode is removed. The QA panel now exposes the real rule viewport independently through wide X/Y, pixel width, and pixel height ranges, while vector thickness, wave amplitude/frequency, and shedding body height/length/dispersion/reach retain independent controls with substantially higher ceilings. The 2026-08-21 load defaults reproduce the user's irregular-wave screenshot (24 fluidity, 2.65× speed, 234/−23 offset, 474×8 viewport, 4.6× thickness, 8× amplitude, 1.7× frequency), and the panel starts minimized. Irregular and regular waves retain their full travelling deformation through each endpoint; tangent-following cubic caps now round the ends without flattening a visibly static landing zone into the moving wave. WebGL and vector surfaces composite transparently over the page; the WebGL shader discards empty fragments and premultiplies antialiased edge colour so transparency does not depend on browser-specific canvas compositing. The mark moves with the pointer field. All modes pause offscreen, during featured-media transitions, and under reduced motion. The remaining design question is whether this mark has a coherent site-wide grammar; interesting shader behavior alone is not approval.
- A14. READY FOR HUMAN QA: testimonials no longer animate in or out. The title uses the selected pointer-proximity field and is explicitly kept on one line in the desktop/tablet side column so per-letter wrappers cannot split it at arbitrary positions. The CMS-selected background texture now moves as the requested shallow scroll-depth plane, with an enable toggle and strength control. Each quote carries a larger low-contrast Bodoni opening mark behind the text; the QA alternative is now a slow continuous color wash rather than transplanting the striped bar material onto the quotation. Reduced motion freezes texture, bars, quote wash, and heading field. The deliberate overlap still needs a readability decision: the current mark sits behind live text and adds layering, but that same overlap can reduce local contrast.
- A16. READY FOR HUMAN QA: small section-level ambient candidates now have independent controls rather than being presumed decisions. The broader occasional Selected Work blue current remains under consideration. Human review rejected and removed the image-edge runners, idle catalog peek, and text-plate travelling signal. The ordinal star is larger and slightly elliptical so its rotation is legible without the rejected satellite dot; four new ordinal-scale candidates now compare a travelling wavy current, orbiting dots, breathing brackets, and an asymmetric rotating dial. The travelling wave's dash phase closes on its pattern period for a seamless loop, and ordinal transition clipping is limited to the actual transition state so the bracket candidate may break frame at rest. These stay close to the card's existing catalog identity and deliberately echo the Selected Work rule / Latest Writing crosshair vocabulary instead of laying more unrelated effects over the photographs. Latest Writing can slowly rotate its crosshair while retaining the existing optional low-travel bento proximity field. The footer can compare a sparse travelling top-edge signal and a repeated ticker treatment of its real CMS-authored “Bottom line, still up front.” heading; the ticker remains clipped inside the original heading column without changing the two-column footer composition or adding blue rules. All autonomous candidates stop under reduced motion. The Motion QA panel is wider, viewport-height limited, internally scrollable, and grouped into native disclosure sections so its minimize control remains reachable. Judge the five ordinal candidates independently rather than enabling them all as a final composition.
- A17. READY FOR HUMAN QA: a self-hosted pixel-art construction GIF now appears in a fixed yellow-and-black “Under construction / Live preview” banner on every route so an unfinished animation phase can coexist with a public production launch. It is enabled by default, has a Motion QA toggle during development, sits at the top-left, compacts on phone, can be dismissed with a real button, and is removed from print. It began as temporary launch chrome, but human review found it charming enough that permanent use is now an open art-direction choice rather than a foregone removal.
- A18. READY FOR HUMAN QA: the homepage BLUF pill now carries a continuously rotating slightly elliptical star, without a satellite ornament, and is an explicit button disclosure control. Opening it explains “Bottom Line Up Front” as clarity-first, depth-on-demand working practice and provides jump links to Selected Work, Collaborators, Side Projects, and Latest Writing. The Testimonials section now exposes the `collaborators` anchor. Its phone layer now matches the desktop disclosure layer so the panel paints above the bespoke hero composition rather than opening invisibly behind it. Verify the copy, disclosure placement across the bespoke desktop/tablet/phone hero compositions, keyboard behavior, and whether the jump menu is useful enough to retain.
- A16. READY FOR HUMAN QA: Latest Writing has an optional extremely low-travel pointer-proximity field on the bento card wrappers, with a toggle and 0–4× strength control. The field is disabled by default, preserves card internals and transition hooks, clears during featured-media transitions, ignores touch, and becomes static under reduced motion. Side Projects now applies the selected heading pointer field to its large linked title; that title is sourced from a new front-page ACF field exposed as `homepageSideProjectsHeading`, with the existing copy as a frontend fallback. Verify whether bento depth adds life without making the card grid feel loose, and verify Side Projects wrapping on desktop, tablet, and phone after authoring CMS copy.
- A15. BRAINSTORM: make the Selected Work accent intentional through repetition rather than defending it as a one-off. Candidate family placements are testimonial signal bars, article metadata punctuation, and occasional section eyebrows where a real structural accent already belongs. Repetition should share material behavior, scale logic, or pointer response without requiring several WebGL canvases or adding marks to surfaces that do not need punctuation.
- A8. READY FOR HUMAN QA: the baked-seed margin organisms now run in the empty desktop margins of real writing-detail pages using the homepage RD's pale `#cddeff` display treatment and concentration thresholds. Their broad fade is rounded again, but moving ecological hostility keeps growth away from that last-resort mask. Dead zones are less destructive, frequent blossoms and a drifting mature-seed reservoir continually reform coral instead of allowing extinction, and pointer proximity applies a stronger patchy fertility boost. Simulations pause during featured-media transitions and resume 600ms after the transition state clears. They are hidden below 1200px. Verify edge disappearance, sustained morphology, hover growth, prose/TOC layering, and desktop performance.
...
## Ready For Human QA
- A0. BRAINSTORM IN PROGRESS: expand the candidate space before implementation. The agreed next vehicle is a fake-content `/dev/motion` lab with a floating debug control: all sample case-study cards use the same selected hover treatment rather than assigning a different variant to each card. Preserve the full comparison slate—inner-image parallax, moving partitions, print misregistration, aperture reveals, clipped catalog/metadata shuffles, and project-specific animated signals—alongside restrained SVG-filter/clip/mask motion, per-letter display-heading parallax, purpose-built animated glyphs, an RD eyebrow, one-time staggered entrances for writing rows and testimonials, and sparse interior RD patches. The Side Projects page is not a list-reveal target. Judge each candidate by character, coexistence, legibility, performance, reduced-motion behavior, and whether it adds a genuinely different kind of motion from RD/Conway. Do not build the lab until the Android RD phone-QA loop works. — IMPLEMENTED 2026-08-12. `/dev/motion` began as a CMS-independent comparison surface and keeps production cards and transition hooks untouched. The first QA pass showed that abstraction was obscuring whether the effects belonged on the actual site, so the revised lab now loads real CMS case studies, reuses the production Selected Work card compositions and baked halftone media, and measures its effect layers against the real image geometry. The floating control still applies one shared hover grammar across all three cards; catalog mode now proposes meaningful project-discipline metadata and activates by viewport position on touch devices. Text testing now includes two pointer-parallax geometries and a scroll-entry cascade, while the ooze control reaches deliberately excessive displacement values so aliasing and readability limits are visible. Inset depth responds independently to scroll and pointer, with optional slight tilt. Writing rows and testimonials now use their production components and trigger a one-time slit-style stagger on viewport entry rather than page load. The RD eyebrow and transparent margin organisms are actual offscreen-paused CPU Gray-Scott simulations; the organisms use tall fields and masked, lava-like drift instead of an opaque periwinkle bar. Reduced motion freezes or removes all autonomous and interaction motion. On a phone the controls begin collapsed. First human QA selected catalog shuffle for eventual production integration, kept print registration and inset parallax under consideration, and rejected moving partitions, aperture reveal, project signal, and the atlas glyph. The next lab revision makes the cards and writing specimen viewport-width like their destinations; restores the real Selected Work photo-treatment provider and adds an on/off radio for the existing duotone-to-color hover; separates the formerly ambiguous global intensity into named controls for card effects, case-image travel/tilt, letter parallax, inset scroll/pointer/tilt, and entrance stagger; adds center-pivot and true flat-plane letter geometry; makes scroll cascade independently composable with pointer parallax; shows ooze on homepage-scale Selected Work and Latest Writing heading compositions; adds selectable row/testimonial entrance grammars; and removes the organisms' external breathing/panning loop so visible change comes from reaction-diffusion growth, destruction, and a moving terrain field. Second human QA removed partitions, aperture, and project signal from the controls; made image parallax independent so catalog+parallax and registration+parallax can be compared; required registration to preserve the production duotone-to-full-colour interaction; increased catalog hierarchy and internal stagger; replaced the abstract inset scene with real case-study media; retained combined second-letter pointer parallax plus scroll cascade; added an oozing graphic accent as an alternative to distorted type; split writing and testimonial entrance choices and replaced the center slit with directional top/bottom slits; and moved both RD specimens onto mature homepage UV seeds. Third human QA selected catalog+parallax+colour reveal, defined the two catalog metadata roles, requested an original-image comparison, approved bottom-slit writing and lateral-wipe testimonials, and rejected the standalone clipped-inset specimen. The lab now exposes original versus baked media; registration displacement follows pointer/parallax distance without scaling its overlay; the ooze glyph sits on the production-like Selected Work heading; margin organisms run faster with broader fades; and the eyebrow has returned to raw RD concentration plus moving fertility terrain instead of a stamped display mask. Fourth human QA approved engagement context as the catalog's second CMS field, rejected print registration, requested a production-context source comparison, and held broader rollout until the remaining lab candidates are clearer. The next revision separates source from treatment: CMS-original pixels retain the configured duotone and the existing full-colour hover reveal. Case-study depth is now image-only and eases home through the same friction loop; text no longer moves. The letter effect also uses a single animation-frame smoothing loop instead of stacking CSS transitions on pointer events. SVG displacement adds a high-resolution filter surface plus a small final blur to reduce jagged edges. The approved writing and testimonial entrances are removed from the lab, the eyebrow display is half-height, and margin organisms use smaller simulation grids with smooth compositor drift instead of low-resolution integer-pixel jumps. The latest human pass provisionally locks the combined scroll cascade and second-letter pivot and removes it from the lab pending homepage-context QA. Parallax now derives enough scale overscan from its configured maximum travel to avoid exposing the clipped image boundary. After a vector-path detour proved too regular, the accent rule returns to a real animated turbulence/displacement map rendered in a much larger internal coordinate space with pre/post smoothing to preserve oozy irregularity without the earlier jagged raster edge. Both RD accents start from baked homepage state and run accelerated ecological turnover. **Review decision 2026-08-20:** baked halftone, catalog metadata, image parallax, colour reveal, and pointer-proximity headings pass; original-image comparison, alternate heading geometries, broad homepage viewport entry/exit, and testimonial reveal are removed. Human QA remains required for the restored accent-rule materials, RD morphology, article/TOC-layer testing, testimonial ambient details, and final transition handoff.
