...
- A0. BRAINSTORM IN PROGRESS: expand the candidate space before implementation. Explore shallow scroll/pointer parallax, editorial ticker bands, scroll-driven display headings, more expressive case-study hover choreography, restrained squiggle/line motion, staggered list entrances, and sparse interior RD patches. Judge each candidate by character, coexistence, legibility, performance, reduced-motion behavior, and whether it adds a genuinely different kind of motion from RD/Conway.
...
## Ready For Human QA
- **B11 — Separate desktop ambient drift, touch ambient drift, tilt translation, and tilt-driven reaction deformation.** The current report is the opposite of B10's inherited premise: desktop drift is healthy, while mobile tilt produces a strobing tiger-stripe effect. `DRIFT_SPEED_POINTER` remains at the user's current `0.7`; touch autonomous drift starts at `0`; tilt translation has its own `TILT_DRIFT_MAX_OFFSET`, `TILT_DRIFT_EASE`, and `TILT_DRIFT_MAX_SPEED`; and `TILT_REACTION_STRENGTH` independently gates the anisotropic diffusion/advection/rate/fertility machinery that historically creates stripes. Reaction deformation starts at `0`, so tilt moves only the fertility field. The `/dev/rd` harness mirrors the input-specific ambient defaults and exposes reaction strength separately for live phone tuning. On the real phone homepage, hold the device still, tilt it slowly in each direction, reverse direction, and make one sharper movement. Confirm: there is no autonomous field translation while the phone is still; tilt produces a calm bounded drift rather than flashing or tiger stripes; reversal starts promptly enough to read as responsive; and the pattern remains recognizably coral. Then open `/dev/rd`, confirm `tilt:` reports events, and use `tilt max speed`, `tilt offset`, and `reaction strength` independently. Keep reaction strength at `0` for the baseline check.
- **B12 — Local phone preview without static generation/CDN deploy.** `corepack pnpm start:phone` starts the CMS stack and a LAN-facing Nuxt HTTPS server with a printed QR code; `corepack pnpm start:frontend:phone` starts only the frontend when the CMS is already running. This uses a temporary local certificate and stays on the shared LAN; it does not create an external tunnel. Stop the existing foreground Nuxt process, run `corepack pnpm start:phone`, scan the QR code on a phone connected to the same LAN, accept the temporary local certificate warning if prompted, and open `/dev/rd`. Confirm the page loads, the controls are usable, and orientation events arrive without a static build or Bunny deploy. If the browser still withholds orientation events, report the phone/browser and whether `tilt: no events` remains visible.
