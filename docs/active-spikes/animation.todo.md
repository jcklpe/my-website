...
**Current priority, updated 2026-08-12 — START HERE IF YOU ARE PICKING THIS UP COLD.**

B, C, and D shipped. Android reaction-diffusion tilt is approved, the accountless phone-preview workflow remains in Human QA, and Thread A is being narrowed through the CMS-backed `/dev/motion` comparison lab. Catalog metadata, the writing-row entrance, and the testimonial entrance have visual approval but still await production integration; other candidates remain in lab QA.
...
- A5. SELECTED FOR PRODUCTION 2026-08-12, integration pending: add the catalog-shuffle hover to Selected Work using two approved CMS-authored fields rather than decorative labels: **practice/discipline** for the work performed and **engagement context** for the situation or intervention. Preserve the existing card-to-detail measurement geometry, retain a switchable comparison with the current duotone-to-color hover while tuning, and settle the touch behavior; the current lab activates the centered card as a plausible mobile model.
...
- A5. SELECTED IN HUMAN QA 2026-08-12: writing archive rows use a one-time bottom-up slit entrance with a 140ms item stagger. Production implementation must coordinate normal page arrival and forward/reverse featured-media transitions; the lab approval settles the visual grammar, not that integration yet. Removed from `/dev/motion` after approval so the lab stays focused on unresolved choices.
- A6. SELECTED IN HUMAN QA 2026-08-12: homepage testimonials use the lateral-wipe entrance with the lab's shared 140ms item stagger as the current approved timing. Production implementation remains pending; do not substitute the rejected center slit. Removed from `/dev/motion` after approval so the lab stays focused on unresolved choices.
- A7. SELECTED IN HUMAN QA 2026-08-12, DO NOT INTEGRATE YET: the combined scroll cascade and second-letter pointer pivot is the provisionally locked Selected Work heading style. It has been removed from `/dev/motion`; final approval requires testing that exact combination on the actual homepage heading alongside the page's other motion.
- A8. QUEUED, DO NOT INTEGRATE YET: test the margin organism on a real writing-detail page only after the lab specimen reproduces enough of the homepage RD field drift and the edge/morphology treatment is approved.
