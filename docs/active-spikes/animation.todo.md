...
- A0. BRAINSTORM IN PROGRESS: expand the candidate space before implementation. Explore shallow scroll/pointer parallax, article-metadata ticker bands, scroll-responsive display headings without pinning or snapping, per-case-study structural hover variants, restrained SVG-filter/clip/mask/registration motion on display text or accent glyphs, one-time staggered entrances for writing rows and testimonials, tiny RD windows used as structural accents, and sparse interior RD patches. The Side Projects page is not a list-reveal target. Judge each candidate by character, coexistence, legibility, performance, reduced-motion behavior, and whether it adds a genuinely different kind of motion from RD/Conway.
...
## Ready For Human QA
- **B11 — Separate desktop ambient drift, touch ambient drift, tilt translation, and tilt-driven reaction deformation.** The current report is the opposite of B10's inherited premise: desktop drift is healthy, while mobile tilt produces a strobing tiger-stripe effect. `DRIFT_SPEED_POINTER` remains at the user's current `0.7`; touch autonomous drift starts at `0`; tilt translation has its own `TILT_DRIFT_MAX_OFFSET`, `TILT_DRIFT_EASE`, and `TILT_DRIFT_MAX_SPEED`; and `TILT_REACTION_STRENGTH` independently gates the anisotropic diffusion/advection/rate/fertility machinery that historically creates stripes. Reaction deformation starts at `0`, so tilt moves only the fertility field. The first separated starting cap (`0.025`) read as no response on the real phone, so the translation cap is back to the previously legible `0.05` (about an eight-second full reversal) now that reaction deformation is independently off. The phone-preview homepage exposes an explicit sensor-permission/status chip, and `/dev/rd` exposes the same permission path plus the numeric event readout and live controls. On the real phone homepage, tap **Enable tilt QA** if shown, wait for **Tilt QA: active**, then hold the device still, tilt it slowly in each direction, reverse direction, and make one sharper movement. Confirm: there is no autonomous field translation while the phone is still; tilt produces a calm bounded drift rather than flashing or tiger stripes; reversal reads as responsive; and the pattern remains recognizably coral. Then open `/dev/rd`, confirm `tilt:` reports events, and use `tilt max speed`, `tilt offset`, and `reaction strength` independently. Keep reaction strength at `0` for the baseline check.
- **B12 — Local phone preview without static generation/CDN deploy.** `corepack pnpm start:phone` starts the CMS stack and a LAN-facing Nuxt HTTPS server with a printed QR code; `corepack pnpm start:frontend:phone` starts only the frontend when the CMS is already running. This uses a temporary local certificate and stays on the shared LAN; it does not create an external tunnel. The first version left all WordPress media pointed at an HTTP hostname only the Mac could resolve, so the phone showed no CMS images; phone-preview mode now rewrites public upload URLs to a restricted same-origin Nuxt proxy. Stop the existing foreground Nuxt process, run `corepack pnpm start:phone`, scan the QR code on a phone connected to the same LAN, and accept the temporary local certificate warning if prompted. Confirm the CMS portrait and visible card images load, the homepage tilt-status chip is usable, and `/dev/rd` receives orientation events without a static build or Bunny deploy. If the chip reports that HTTPS is not trusted, report the phone/browser because the temporary certificate itself is blocking the sensor API.
