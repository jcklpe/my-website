...
- A13. READY FOR HUMAN QA: the Selected Work production rule is narrowed to three serious candidates: SVG irregular flag, WebGL shedding lava, and an SVG flag with vector shedding. Desktop defaults remain 24 fluidity, 2.65× speed, 234/−23 offset, 474×8 viewport, 4.6× thickness, 8× amplitude, and 1.7× frequency. Tablet and phone now carry independent responsive position/viewport/amplitude defaults matching the approved screenshots: tablet 71/−11, 206×8, 4.7× amplitude; phone −2/−23, 250×8, 5.6× amplitude. The active breakpoint is explicit in the QA labels. The WebGL particles now reach zero mass at both lifecycle boundaries and fade before the canvas edge, removing the periodic reset judder and hard clipping. The hybrid's emitted vector blobs share the same thickness/length/dispersion/reach controls. The mark no longer moves with pointer proximity. All modes pause offscreen, during featured-media transitions, and under reduced motion. Final choice remains irregular flag versus repaired lava versus hybrid, plus the broader question of a coherent site-wide accent grammar.
- A14. READY FOR HUMAN QA: testimonials no longer animate in or out. The title uses pointer proximity and remains one line in the desktop/tablet side column. The CMS-selected texture now responds to pointer position on mouse devices and calibrated device orientation on supporting touch devices instead of subtle scroll depth. Cards have more internal whitespace; the Bodoni quotation mark moves farther into that gutter and its slow blue wash is stronger. The accepted travelling striped block accent remains visible alongside the quotation wash rather than being substituted away. Reduced motion freezes texture, bars, quote wash, and heading field. The deliberate quote/text overlap still needs a final readability judgment.
