...
- A9. READY FOR HUMAN QA: Selected Work now combines the approved catalog shuffle, existing colour reveal, inset-image parallax, and baked halftone in production context. **Practice/discipline** and **engagement context** are explicit ACF fields on Case Study edit screens and GraphQL fields on the `CaseStudy` type. Empty fields remain empty; development no longer invents catalog labels. Pointer travel is capped at 28px with geometry-derived overscan, the outer transition hooks remain fixed, and touch activates a substantially centered card. The CMS-original comparison was removed after the baked halftone passed second-person review. Verify real authored metadata, mobile activation, and card-to-detail transition handoff.
...
- A13. READY FOR HUMAN QA: the Selected Work production rule still compares seven rendering/textural approaches. `HomeFluidAccentRule.vue` retains the crisp filled-SVG fluid ribbon and two travelling-wave constructions; vector modes now expose a 0.5–4× thickness control in addition to amplitude/frequency where relevant. `HomeWebglAccentRule.vue` supplies supersampled procedural flow, slow lava metaballs, domain warp, and shedding lava. The shedding mode no longer begins from a full-width horizontal baseline: independent lobes stretch through temporary bridges, open moving negative space, and release four particles across a wider vertical/horizontal field while shrinking. The mark sits closer to the heading and moves with the pointer field. All modes pause offscreen, during featured-media transitions, and under reduced motion. The remaining design question is whether this mark has a coherent site-wide grammar; interesting shader behavior alone is not approval.
- A14. READY FOR HUMAN QA: testimonials no longer animate in or out. The title uses the selected pointer-proximity field, each quote carries a large low-contrast Bodoni opening mark behind the text, and each card's existing blue striped accent continuously travels horizontally at a quiet linear pace. Reduced motion freezes the stripes and the heading field. Verify quotation clarity, contrast, card hierarchy, and whether the perpetual signal is quiet enough beside the homepage RD.
