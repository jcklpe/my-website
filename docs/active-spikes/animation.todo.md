...
- A16. READY FOR HUMAN QA: small section-level ambient candidates now have independent controls rather than being presumed decisions. The nearly invisible paired-line Selected Work current is replaced by a broader occasional blue light sweep. Four additional case-study comparisons are independently toggleable: a rotating star beside each real ordinal, opposed edge runners on the image plate, a partial ambient catalog-tab peek, and a short signal travelling along the text-plate edge. Latest Writing can slowly rotate its crosshair while retaining the existing optional low-travel bento proximity field. The footer can compare a sparse travelling top-edge signal and a repeated ticker treatment of its real CMS-authored “Bottom line, still up front.” heading; the ticker now remains clipped inside the original heading column without changing the two-column footer composition or adding blue rules. All autonomous candidates stop under reduced motion. Judge whether each loop clarifies the section's existing grammar or merely adds activity.
- A17. READY FOR HUMAN QA: a self-hosted pixel-art construction GIF now appears in a fixed yellow-and-black “Under construction / Live preview” banner on every route so an unfinished animation phase can coexist with a public production launch. It is enabled by default, has a Motion QA toggle during development, stays clear of the right-side QA panel, compacts on phone, and is removed from print. Treat this as deliberately temporary launch chrome rather than a new permanent site component.
