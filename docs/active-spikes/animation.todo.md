...
- A16. READY FOR HUMAN QA: small section-level ambient candidates now have independent controls rather than being presumed decisions. Selected Work can show an occasional paired blue current along case-study image edges; Latest Writing can slowly rotate its crosshair while retaining the existing optional low-travel bento proximity field; the footer can compare a sparse travelling top-edge signal and a repeated ticker treatment of its real CMS-authored “Bottom line, still up front.” heading. All autonomous candidates stop under reduced motion. Judge whether each loop clarifies the section's existing grammar or merely adds activity.
