...
**Current priority, updated 2026-08-31 — START HERE IF YOU ARE PICKING THIS UP COLD.**

B, C, and D shipped. Android reaction-diffusion tilt is approved. Pointer proximity is the selected heading model on Selected Work, Latest Writing, Testimonials, and Side Projects, but the Selected Work accent is deliberately stable rather than joining the letter field. Catalog metadata, inset-image parallax, colour reveal, baked halftone, the broad case-study current, low-travel Latest Writing card proximity, Latest Writing crosshair rotation, footer quiet signal, and footer heading ticker are accepted production behavior rather than QA toggles. The accent-rule comparison is narrowed to irregular flag, repaired shedding lava, and their curve-shedding hybrid. Rotating star and travelling wave are the leading ordinal finalists; breathing brackets remain available but trail them. Article margin organisms are rejected and removed from writing pages because their independent CPU simulation could neither match the homepage GPU ecology nor meet the transition/performance budget. Writing-row and featured-media transition refinements remain in Human QA.

The broad brainstorming/lab phase has done its job. Preserve the user’s goal that the site feel alive—possibly like diving deeper into a living atlas—but do not reopen rejected experiments merely to add more motion. Closeout work should tune accepted behaviors in production context, settle the remaining accent and ordinal comparisons, find one convincing non-pointer ambient behavior for Latest Writing if needed, verify transitions, and then fold durable motion rules into the project docs.
...
- A13. READY FOR HUMAN QA: the Selected Work production rule is narrowed to three serious candidates: SVG irregular flag, WebGL shedding lava, and an SVG flag that sheds vector slime from moving curves along its full length. Wide-desktop flag defaults remain 24 fluidity, 2.65× speed, 234/−23 offset, 474×8 viewport, 4.6× thickness, 8× amplitude, and 1.7× frequency. A new compact-desktop state below 1892px uses the reviewed 100/−23, 474×8 geometry rather than forcing the wide offset through every single-line composition; the threshold is set from the actual point at which the wide rule clears the portrait. Tablet and phone retain their approved independent defaults: tablet 71/−11, 206×8, 4.7× amplitude; phone −2/−23, 250×8, 5.6× amplitude. Standalone lava now keeps a separate reviewed preset—4.25× speed, 279/0, 506×32 on wide desktop, 1× height, 0.85× body length, 1.6× dispersion, and 1× reach—so switching textures no longer destroys the flag's geometry. Its body begins farther inside the domain and tapers through a left-edge alpha ramp; emitted pieces still reach zero mass at lifecycle boundaries and fade before the other canvas edges. The hybrid now emits across the rope, derives throw direction and size from local curve movement, and exposes density and whip-force controls. The mark remains independent of pointer proximity. All modes pause offscreen, during featured-media transitions, and under reduced motion. Final choice remains irregular flag versus repaired lava versus curve-shedding hybrid.
- A14. READY FOR HUMAN QA: testimonials no longer animate in or out. The title uses pointer proximity and remains one line in the desktop/tablet side column. The CMS-selected texture now responds to the pointer's viewport position even while the pointer is outside the section; touch devices retain calibrated projected-orientation control. Cards have more internal whitespace. The quotation signal is now a closing mark anchored in the bottom-right corner, underneath the content layer but outside the quote body, so it remains typographically present without interrupting reading. Its slow blue wash remains stronger. The accepted travelling striped block accent remains visible alongside the quotation wash rather than being substituted away. Reduced motion freezes texture, bars, quote wash, and heading field. Verify the new corner placement across short/long testimonials and the global pointer mapping.
- A16. READY FOR HUMAN QA: the broad occasional blue current is accepted and locked on for Selected Work cards. Orbiting dots and the rotating dial are rejected and removed. The remaining mutually exclusive ordinal options are a corrected symmetric SVG star rotating around its true optical center, the travelling wavy current, and narrower breathing brackets with a snappier easing curve at the same slow tempo. Latest Writing's extremely low-travel card proximity and rotating crosshair are accepted and locked on; the strength control remains for tuning. The footer quiet signal and CMS-heading ticker are also accepted and locked on. Motion QA now exposes decisions still requiring comparison rather than toggles for settled behavior.
...
- A19. READY FOR HUMAN QA: Latest Writing's card proximity is enabled by default, preserves card internals and transition hooks, clears during featured-media transitions, ignores touch, and becomes static under reduced motion. Side Projects applies the selected heading pointer field to its CMS-backed large linked title. Verify whether the accepted low travel stays quiet enough across the bento composition and verify Side Projects wrapping on desktop, tablet, and phone after authoring CMS copy.
