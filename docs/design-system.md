...
The article table of contents is another article apparatus layer, but on the left side. `ArticleToc.vue` is inserted through `BlockRenderer`'s apparatus slot inside `.content-flow`, scans rendered `h2` through `h6` headings, and suppresses itself on short articles. Desktop uses a low-priority left rail that starts open when unobstructed, tracks the active section, and auto-collapses after `200vh` of accumulated reader scrolling in either direction; manually reopening it resets that allowance. A meaningful overlap during settled page entry latches the rail hidden until the painted obstacle clears completely; subsequent scrolling uses the more tolerant overlap heuristic for temporary crossings. Phone/tablet uses an in-flow collapsed Contents block aligned to the content column; it is not a fixed or floating mobile control. When a mobile TOC link is selected, close and settle that in-flow list before measuring and smooth-scrolling to the heading, because collapsing the list after navigation begins moves the target away from the position the browser already chose.

Authored content wins over the TOC. Wide, full, floated, and compositional blocks paint above the rail rather than being pushed around by it. When the TOC underlaps those objects, use neutral cream clearing so the overlap reads as intentional paper layering instead of clutter. Article `BlockRenderer` instances that carry the TOC explicitly enable this context; other content flows, including Side Projects, leave it transparent, and the WordPress editor omits it. The shared `underlap-matte` mixin in `packages/styles/_mixins.scss` provides a 25px matte with an 8px radius without changing the block's layout footprint. Matte painting and TOC collision measurement must follow the same visible surface, not a wider transparent layout shell; height-capped full-width Images therefore apply their matte and obstacle outset to the rendered image rather than the viewport-wide figure. Transparent interiors, such as Media/Text copy, Columns, and Audio, need a real cream background too. If a block has its own non-cream surface, border, or shadow, keep that visible surface above the matte with a foreground surface layer rather than relying on the parent background.
