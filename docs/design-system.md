...
The article table of contents is another article apparatus layer, but on the left side. `ArticleToc.vue` is inserted through `BlockRenderer`'s apparatus slot inside `.content-flow`, scans rendered `h2` through `h6` headings, and suppresses itself on short articles. Desktop uses a low-priority left rail that starts open when unobstructed, tracks the active section, auto-collapses after the reader is underway, and can be manually reopened. A meaningful overlap during settled page entry latches the rail hidden until the painted obstacle clears completely; subsequent scrolling uses the more tolerant overlap heuristic for temporary crossings. Phone/tablet uses an in-flow collapsed Contents block aligned to the content column; it is not a fixed or floating mobile control.
