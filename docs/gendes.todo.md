...
Read `docs/gendes.md` for the full methodology. Read `docs/visual-design.md` for the baseline this work forks from. For exploratory runs, just assemble a mood board and hand it to an agent. For synthesis runs, author `docs/scratch/gendes-brief.md` from audit data first.
...
### Design Branch Workflow — Two Modes

**Exploratory run** (first pass on a new direction):
- Create the branch: `git checkout -b gendes-<direction>` from `gendes-academia`
- Assemble a mood board in `docs/gendes-moodboard/<branch-name>/`
- Hand the mood board to an agent; let it implement without a written brief
- Review the output section-by-section and document findings

**Synthesis run** (after auditing multiple exploratory runs):
- Create the branch from `gendes-academia` or a prior gendes branch
- Author `docs/scratch/gendes-brief.md` from audit notes — specific section-by-section decisions, anti-patterns to avoid, what was learned
- Give the agent the brief; use `git show <branch>:<path>` to pull specific styles from prior runs
