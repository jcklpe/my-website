<script setup lang="ts">
  ...
  // iOS requires requestPermission() inside a user gesture, so this harness has
  // an explicit enable button. Once granted for this origin, navigating back to
  // the homepage lets its existing listener receive the same events.
  ...
  const tiltPermissionLabel = computed(() => {
    switch (tiltPermission.value) {
  ...
  function listenForOrientation() {
    ...
    tiltPermission.value = 'waiting';
  }

  async function enableTilt() {
    if (!window.isSecureContext) {
      tiltPermission.value = 'insecure';
      return;
    }
    ...
    if (!orientation) {
      tiltPermission.value = 'unsupported';
      return;
    }
    ...
    tiltPermission.value = 'requesting';

    try {
      const permission = await orientation.requestPermission();

      if (permission !== 'granted') {
        tiltPermission.value = 'denied';
        return;
      }
  ...
  function initializeTiltInput() {
    ...
    if (typeof orientation.requestPermission === 'function') {
      tiltPermission.value = 'needs-permission';
      return;
    }
    ...
    <div class="rd-dev-panel" :class="{ 'is-collapsed': collapsed }">
      ...
      <div class="rd-dev-head">
        <div>
          ...
          <p class="rd-dev-status" :class="{ 'is-live': tiltEvents }">
            sensor: {{ tiltPermissionLabel }}
            <button
              v-if="
                tiltPermission === 'needs-permission' ||
                tiltPermission === 'denied'
              "
              class="rd-dev-permission"
              type="button"
              @click="enableTilt"
            >
              enable tilt
