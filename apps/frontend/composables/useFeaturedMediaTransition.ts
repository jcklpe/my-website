...
function findMediaFrame(kind: 'source' | 'target', key: string) {
  return document.querySelector<HTMLElement>(`[data-featured-media-${kind}="${key}"]`)
}

function findTitleFrame(kind: 'source' | 'target', key: string) {
  return document.querySelector<HTMLElement>(`[data-featured-title-${kind}="${key}"]`)
}

function findMetaFrame(kind: 'source' | 'target', key: string) {
  return document.querySelector<HTMLElement>(`[data-featured-meta-${kind}="${key}"]`)
}
...
function titleStyleFromElement(element: HTMLElement | null): FeaturedMediaTransitionTitleStyle | null {
  if (!element) {
    return null
  }

  const labelElement = element.firstElementChild instanceof HTMLElement
...
function metaStyleFromElement(element: HTMLElement | null): FeaturedMediaTransitionMetaStyle | null {
  if (!element) {
    return null
  }
...
function millisecondsFromCssTime(value: string) {
  ...
  if (trimmedValue.endsWith('s')) {
    return (Number.parseFloat(trimmedValue) || FALLBACK_TRANSITION_DURATION / 1000) * 1000
  }
...
function routeTransitionDuration() {
  ...
  const routeDuration = getComputedStyle(document.documentElement)
...
export function useFeaturedMediaTransitionState() {
  return useState<FeaturedMediaTransitionState>('featured-media-transition', () => ({
    active: false,
    key: null,
    media: null,
    from: null,
